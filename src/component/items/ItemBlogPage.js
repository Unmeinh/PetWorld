...
const ItemBlogPage = (row) => {
    ...
    function OpenEditBlog() {
        setisShowMenu(false);
        if (row.fetchBlog != undefined) {
            navigation.navigate('EditBlog', { oldBlog: blog, fetchBlog: () => row.fetchBlog() });
        } else {
    ...
    return (
        <Pressable pointerEvents={(isDeleting) ? 'none' : 'auto'}>
            <Animated.View style={(isDeleting) ? { opacity: fadeAnim } : {}}>
                ...
                {
                    (isShowComment)
                        ? <CommentTab isShow={isShowComment} blog={blog} isMe={true} isFollow={isFollowed}
