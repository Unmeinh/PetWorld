...
const listPetSlice = createSlice({
  name: 'listpet',
  initialState: {status: 'idle', pets: [], message: '',petsByIdShop:[],statusPetByIdShop:'idle'},
  reducers: {
...
export const fetchPets = createAsyncThunk('pets/fetchPets', async (page = 1) => {
  const res = await api.get(`/pet/list/all?page=${page}`);
  return res.data;
});
export const fetchPetsByIdShop = createAsyncThunk('pets/fetchPetsByIdShop', async (id) => {
  const res = await api.get(`/pet/list/shop/${id}`);
  return res.data;
});
